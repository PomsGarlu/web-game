...
ws.onmessage = (event) => {
  ...
  if (data.type === "winNotifier") {
    alert(
      `Player ${data.winner} has won the game with a score of ${data.winnerScore}!`
    );
    console.log("stop timer");
    ws.send(JSON.stringify({ type: "timer", status: "stop" }));
    window.location.reload();
  ...
  if (data.type === "gameStart") {
    ...
    players.forEach((player, index) => {
      ...
      if (index < playerData.length) {
        ...
        img.setAttribute(
          "transform",
          `rotate(${playerData[index].rotation} ${playerData[index].x + 13} ${
            playerData[index].y + 26
          })`
        );
